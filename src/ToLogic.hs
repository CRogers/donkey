...
intNP [P "NP" np1, L "CC" ("and",_), P "NP" np2] r vb = intNP np1 r (\x -> intNP np2 r (inner x))
	where inner x y = VQ0 (VSw `VC` VE (x++y) `VC` VP "eq-on-of" [(x++y),x,y] `VC` VSw `VC` vb (x++y)) -- hopefully x+y is free
...
intVP [L "VB" vb, P "NP" np] r = \v -> intNP np r (intVB2 vb r v) -- it could be nice to 'swap' subject and object here, to make it read more like the original sentence
